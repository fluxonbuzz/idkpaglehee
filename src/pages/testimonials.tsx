...
export default function TrustedStaffPage() {
  ...
  const [unlockedStaff, setUnlockedStaff] = useState([]);
  const [selectedBadge, setSelectedBadge] = useState(null);

  // Staff passwords (in a real app, these would be hashed and stored securely)
  const staffPasswords = {
    0: 'founder2023',    // Shiva XD
    1: 'fluxpower',      // Fluxon
    2: 'phantom123',     // Fantom
    3: 'shadow789',      // SilentShadow
    4: 'newrecruit'      // OPEN position
  };

  const handlePasswordSubmit = (staffId) => {
    if (passwords[staffId] === staffPasswords[staffId]) {
      setUnlockedStaff([...unlockedStaff, staffId]);
      setPasswords({...passwords, [staffId]: ''});
    } else {
  ...
  const togglePasswordVisibility = (staffId) => {
    setShowPassword({...showPassword, [staffId]: !showPassword[staffId]});
  };
  ...
  const staffMembers = [
    ...
    },
    {
      ...
      eligibleBadges: ['guardian', 'sentinel']
    },
    {
      id: 4,
      name: '-- OPEN --',
            ...
            {staffMembers.map((member) => (
              <div key={member.id} className="bg-gray-800/50 border border-gray-700 rounded-xl p-6 hover:border-purple-500/50 transition-all">
                ...
                <div className="space-y-4">
                  ...
                  {/* Password Protection for Badge Ranking */}
                  {!unlockedStaff.includes(member.id) ? (
                    <div className="mt-4 pt-4 border-t border-gray-700">
                      <h3 className="text-xs font-semibold text-gray-500 mb-2">STAFF ACCESS REQUIRED</h3>
                      <div className="relative">
                        <input
                          type={showPassword[member.id] ? "text" : "password"}
                          value={passwords[member.id]}
                          onChange={(e) => setPasswords({...passwords, [member.id]: e.target.value})}
                          placeholder="Enter staff password"
                          className="w-full bg-gray-700 rounded-lg px-3 py-2 text-sm pr-10"
                        />
                        <button
                          onClick={() => togglePasswordVisibility(member.id)}
                          className="absolute right-2 top-2 text-gray-400 hover:text-white"
                        >
                          {showPassword[member.id] ? <EyeOff size={16} /> : <Eye size={16} />}
                        </button>
                      </div>
                      <button
                        onClick={() => handlePasswordSubmit(member.id)}
                        className="mt-2 w-full bg-purple-600 hover:bg-purple-700 rounded-lg px-3 py-2 text-sm font-medium transition-colors"
                      >
                        Unlock Badge Ranking
                      </button>
                    </div>
                  ) : (
                    <div className="mt-4 pt-4 border-t border-gray-700">
